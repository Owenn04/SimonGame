...
public class Simon implements ActionListener, MouseListener
{
	...
	public int flashed = 0, dark, ticks, indexPattern, score;
	...
	public void start() //starting variables
	{
		random = new Random();
	...
	public static void main(String[] args) //begin game
	{
		simon = new Simon();
	}

	@Override
	public void actionPerformed(ActionEvent e) //when you click
	{
		ticks++;
	...
	public void paint(Graphics2D g) //create graphics
	{
		...
		{
			g.drawString("You lose!", 75, HEIGHT / 2 + 42);
			
			g.setFont(new Font("Arial", 1, 60));
			g.drawString("Click to play again", 120, 700);
			
			g.setFont(new Font("Arial", 1, 50));
			g.drawString("Your score was " + String.valueOf(score) , 200, 115);
		}
		else
		{
			g.drawString(indexPattern + "/" + pattern.size(), WIDTH / 2 - 100, HEIGHT / 2 + 42);
			g.drawString(String.valueOf(score), 685, 115);
	...
	@Override
	public void mousePressed(MouseEvent e) //track mouse poxxs. Depending on pos flash different colour
	{
